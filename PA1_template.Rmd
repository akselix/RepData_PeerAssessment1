```{r Set global knitr options, echo = F}
library(knitr)
...
`r format (Sys.Date (), '%Y-%d-%m')`


```{r}
# If file exists in working directory, read in data. If file does not exist, download zip file, unzip and read in data without saving the file.
...
```{r}
ggplot(df, aes(date2, steps)) +
    geom_bar(stat = 'identity') +
    scale_x_date(breaks = '1 day', labels = date_format('%m-%d')) +
    labs(title = 'Steps Taken Each day', x = 'Date', y = 'Number of Steps') +
...
**`r round(mean(steps_day),digits = 0)` is the mean value for steps per day**

**`r round(median(steps_day),digits = 0)` is the median value for steps per day**
...
ggplot(data = df, aes(interval_unique, steps_per_interval)) +
    geom_line() +
...
**Interval `r df_interval_sorted[1,1]` has maximum number of steps.**
...
**`r sum(is.na(df))` is the total number of NA's**
...
df_impute <- ddply(df_impute,.(interval), transform, steps_mean = round(mean(steps, na.rm = T), digits = 0))
# Replace NA values with means
df_impute$steps[is.na(df_impute$steps) == T] <- df_impute$steps_mean[is.na(df_impute$steps) == T]
...
```{r}
# Create a histogram
ggplot(df_impute, aes(date2, steps)) +
    geom_bar(stat = 'identity') +
    scale_x_date(breaks = '1 day', labels = date_format('%m-%d')) +
    labs(title = 'Steps Taken Each day', x = 'Date', y = 'Number of Steps') +
    theme(axis.text.x=element_text(size = 6, angle=90))
...
median(steps_day2)
```
**`r round(mean(steps_day2),digits = 0)` is the mean value for steps per day**

**`r round(median(steps_day2),digits = 0)` is the median value for steps per day**
...
df_week$day <- as.factor(ifelse(weekdays(date2) %in% c('Saturday', 'Sunday'), 'weekend', 'weekday'))
```
...
```{r}
# Plot 1 for weekends
# Subset week-end data
df_weekend <- df_week[df_week$day == 'weekend',]
# Create a variable of total number of steps per interval value
steps_weekend <- rowsum(df_weekend$steps, df_weekend$interval)
# Plot the timeseries
plot1 =
ggplot(data = df_weekend, aes(interval_unique, steps_weekend)) +
    geom_line() +
    labs(title = 'Weekend', x = 'Interval', y = 'Steps') +
...
# Plot the timeseries
plot2 =
    ggplot(data = df_weekday, aes(interval_unique, steps_weekday)) +
    geom_line() +
    labs(title = 'Weekday', x = 'Interval', y = 'Steps') +
    scale_y_continuous(limits = c(0, 12000))
